...
function setCurrentDay(weekday, day) {
    // weekday = #column 
    var date = new Date();
    var currentDay = date.getDay(); // با استفاده از تابع getDay، روز هفته فعلی را به دست می‌آوریم

    if (currentDay === weekday) {
        var x = "1-" + weekday;
        document.getElementById(x).innerHTML = day;
    } else if (currentDay + 7 === weekday) {
        var x = "2-" + weekday;
        document.getElementById(x).innerHTML = day;
    } else if (currentDay + 14 === weekday) {
        var x = "3-" + weekday;
        document.getElementById(x).innerHTML = day;
    } else if (currentDay + 21 === weekday) {
        var x = "4-" + weekday;
        document.getElementById(x).innerHTML = day;
    } else if (currentDay + 28 === weekday) {
        var x = "5-" + weekday;
        document.getElementById(x).innerHTML = day;
    }
}
